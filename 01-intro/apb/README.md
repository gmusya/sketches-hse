...
Эта задача предназначена для знакомства с тестирующей системой. Вам предстоит реализовать функцию `Add`, которая складывает 2 числа, в файле `add.cpp`. Перед тем, как приступить к решению, настройте окружение согласно общей инструкции.
...
* `*.cpp` --- файлы, в которых вы должны реализовать то, что требуется в условии задачи (в данном случае `add.cpp`).
...
Когда вы делаете посылку, система запускает тесты, и в случае успешного завершения задача вам засчитывается.

### Сборка

Вы можете использовать любую удобную вам среду разработки или текстовый редактор. Обращайтесь к Yandex о том, как настроить CMake проект в вашей IDE. Ниже будет инструкция по настройке окружения через командную строку.
...
* Отступы в 2 пробела
* Максимальная длина строки в 120 символов
...
```sh
# В директории sketches-hse.
mkdir build
cd build
```

Убедитесь, что у вас уже установлен `cmake` нужной версии, командой `cmake --version`. Версия должна быть не ниже 3.22. Если `cmake` еще не установлен, то нужно сделать это:
...
make test_apb
```
Названия таргетов (в этой задаче `test_apb`) можно брать из файла `.tester.json` из поля `"tests"`.
...
Теперь в директории `build` есть исполняемый файл `test_apb`, который и нужно запустить: `./test_apb`.
...
Чтобы clangd видел этот файл, сделайте симлинк на него в корне репозитория. Находясь в sketches-hse, выполните `ln -s build/compile_commands.json compile_commands.json`, где `build` - имя директории, в которой вы запускали `cmake`.
